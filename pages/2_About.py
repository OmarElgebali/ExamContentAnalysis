...
### **5. Syllabus Determination Options:**
   1. **Option 1:** Generate a topic from a given question, compare it with the model-generated topics using similarity metrics (e.g., USE or Siamese network), and determine syllabus inclusion based on a threshold.
   2. **Option 2:** Transform the question using the model's approach and check if it corresponds to a generated topic. Syllabus inclusion is based on whether the transformed question belongs to the range [0, n-1] or is an outlier topic -1.

"""


# st.markdown("# Exam Content Analysis Module")
st.write(info)
